...
create table if not exists products (
  ...
  category text,              -- e.g. 'Kurti', 'Saree', 'Suit Set'
  fabric text,                -- e.g. 'Cotton', 'Georgette', 'Muslin'
  color text,
...
create table if not exists orders (
  ...
  city text,
  pincode text,
  items jsonb not null,          -- [{id, title, price, qty, size}]
...
-- ---------- SITE CONTENT (editable homepage banner/text) ----------
create table if not exists site_content (
  id uuid primary key default gen_random_uuid(),
  content_key text unique not null,  -- e.g. 'hero_heading', 'hero_subtext', 'hero_image', 'about_snippet'
  content_value text,
...
-- ============================================
-- ROW LEVEL SECURITY
-- Public (anon key) can READ products/content/testimonials/blog,
-- and INSERT orders/newsletter/contact.
-- Only authenticated admin users can write products/content/testimonials/blog
-- and read orders/contact messages.
-- ============================================
...
-- Admin (authenticated) full access — run AFTER creating your admin user in Supabase Auth
...
-- After running this, go to Storage in Supabase Dashboard and confirm
-- a public bucket named 'product-images' exists.
-- ============================================
